/**
 * @file
 * @brief This file contains functions for reading and allocating data from the
 * SAVE directory generated by DFT/DFPT codes (like QE or Yambo), including
 * lattice parameters, wavefunctions (WFCs), and pseudopotential information.
 */
...
/*static functions */
/**
 * @brief Reads the entire data of a specified variable from an open NetCDF file
 * in serial mode.
 *
 * @note This function performs a serial read and should only be called by a
 * single process, typically `Comm->commW_rank == 0`.
 *
...
/**
 * @brief Allocates and sets the G-vectors (plane wave vectors) for a specific
 * k-point.
 *
 * It uses the global list of G-vectors (`totalGvecs`) and k-point specific
 * indices
 * (`Gvecidxs`) to retrieve the local G-vectors and converts them from
 * crystallographic coordinates (in units of $2\pi/a_i$) to cartesian
 * coordinates (in units of $2\pi/L_i$) using the lattice parameters.
 *
 * The local G-vectors are stored in `gvec` in the order of (nG, 3).
 * The cartesian G-vector $\mathbf{G}_{loc}$ is calculated as:
 * $$
 * \f[
 * \mathbf{G}_{loc}[i] = \frac{\mathbf{G}_{total}[Gidx][i]}{lat\_param[i]}
 * \f]
 * $$
 * where $Gidx$ is the index into the `totalGvecs` array.
 *
 * @param gvec Pointer to the output buffer for local G-vectors (nG, 3).
 * @param ik Index of the k-point in the irreducible BZ (iBZ).
 * @param totalGvecs Global array of all G-vectors (ng_total, 3).
 * @param ng_total Total number of unique G-vectors across all k-points.
 * @param Gvecidxs Indices mapping total G-vectors to k-point G-vectors (nibz,
 * ng_shell).
 * @param ng_shell Max number of G-vectors for any k-point.
 * @param lat_param Lattice parameters $a, b, c$ (3 elements).
 * @param nG Number of G-vectors for the current k-point on the current CPU
 * slice.
 * @param nG_shift Starting index (offset) of the G-vectors for the current CPU
 * slice.
 */
...
/**
 * @brief Reads a sub-array (slice) of a specified variable from an open NetCDF
 * file.
 *
 * This function uses parallel NetCDF I/O (`nc_open_par`, `nc_var_par_access`,
 * `nc_get_vara`) and performs a **collective** read operation.
 *
 * @param ncid NetCDF file ID.
 * @param var_name Name of the variable to read from.
 * @param startp Array of starting indices for the slice (NetCDF dimension
 * order).
 * @param countp Array of counts (extent) for the slice (NetCDF dimension
 * order).
 * @param data_out Pointer to the buffer where the data slice will be stored.
 */
static void quick_read_sub(const int ncid, char* var_name, const size_t* startp,
                           const size_t* countp, void* data_out);

/**
 * @brief Reads the plane-wave coefficients (wavefunctions) for a specific
 * k-point.
 *
 * This function opens the k-point specific NetCDF file fragment, reads the
 * required bands/spin components and plane-wave slice for the current process,
 * and stores them in `out_wfc`. It uses parallel NetCDF I/O.
 *
 * The read slice dimensions correspond to:
 * (bands, nspinor, npw_loc, 2 [real/imag])
 *
 * @param spin_stride_len The stride length in the output buffer for a single
 * spin component: $\text{nbnds} \times \text{nspinor} \times \text{nG}$.
 * @param ik Index of the k-point in the irreducible BZ (iBZ).
 ...
 * @param start_band Starting band index (1-based, inclusive) to read.
 * @param nbnds Number of bands to read: $\text{end\_band} - \text{start\_band}
 * + 1$.
 * @param nG Number of G-vectors for the current k-point on the current CPU
 * slice.
 * @param G_shift Starting index (offset) of the G-vectors for the current CPU
 * slice (0-based).
 * @param save_dir Path to the SAVE directory.
 * @param work_array Temporary buffer for path/variable names.
 * @param work_array_len Length of the `work_array` buffer.
 * @param out_wfc Output buffer for wavefunctions (nspin, bands, nspinor, nG).
 * @param comm MPI Communicator for G-vector parallelization (`Comm->commK`).
 */
...
/**
 * @brief Determines which MPI pool is responsible for reading the wavefunction
 * data for a given k-point.
 *
 * This function implements the distribution logic for I/O tasks based on
 * k-point index.
 *
 * @param ik K-point index in the irreducible BZ (0-based).
 * @param q Quotient from $N_{iBZ} / N_{pools}$.
 * @param r Remainder from $N_{iBZ} / N_{pools}$.
 * @return The rank of the I/O pool (`Comm->commR_rank`) responsible for reading
 * `ik`.
 */
 ...
 *
 * @param arr_in Pointer to the input array of integers.
 * @param nelements Number of elements in the array.
 * @param check_ele The integer value to check for existence.
 * @return `true` if `check_ele` is found, `false` otherwise.
 */
...
/* Function body */
/**
 * @brief Allocates memory and reads all necessary data (lattice, pseudo, WFCs)
 * from the DFT SAVE directory.
 *
 * This function performs an initial setup by reading fundamental parameters
 * (lattice vectors, symmetries, k-points, pseudopotentials, WFC grid) from
 * NetCDF files in the `SAVEdir`. This reading is done serially by the rank 0
 * process of `Comm->commW` and then broadcasted. Wavefunctions (WFCs) and
 * Kleinman-Bylander (KB) projectors (`Fk`) are read in parallel across I/O
 * pools (`Comm->commR`) and then broadcasted to all processes.
 *
 * Bands are specified using **1-based** indices (Fortran convention).
 *
 * @param SAVEdir Path to the DFT SAVE directory (read-only data, e.g.,
 * wavefunctions).
 * @param Comm Pointer to the structure containing MPI communicators and pool
 * sizes.
 * @param start_band The first band index (1-based) for which data will be read.
 * @param end_band The last band index (1-based, inclusive) for which data will
 * be read.
 * @param wfcs Pointer to a pointer to the array of WFC structures to be
 * allocated and filled (iBZ size).
 * @param ph_save_dir Path to the DFPT SAVE directory (may contain
 * pseudopotentials).
 * @param lattice Pointer to the Lattice structure to be filled.
 * @param pseudo Pointer to the Pseudo structure to be filled.
 * @param phonon Pointer to the Phonon structure (used to initialize Vloc
 * table).
 * @param dft_code Enum indicating the origin of the SAVE directory data (e.g.,
 * QE).
 */
                              ...
                              enum ELPH_dft_code dft_code)
{
    ...
    */
    ...
    if (Comm->commW_rank == 0)
    {
        ...
        nkBZ = (int)rint(kindx_pars[0]);  // Total k-points in full BZ
#else
        int nkBZ_read;
    ...
    lattice->nspinor =
        rint(dimensions[11]);  // Number of spinor components (1 or 2)
    lattice->nspin =
        rint(dimensions[12]);  // Number of spin components (1 or 2)
    lattice->timerev = rint(dimensions[9]);  // Time-reversal symmetry flag
    lattice->total_bands =
        rint(dimensions[5]);  // Total number of bands available in SAVE
    lattice->nsym = rint(dimensions[10]);  // Total number of symmetries
                                           // (including time reversal)
    lattice->nkpts_iBZ =
        rint(dimensions[6]);  // Number of k-points in irreducible BZ
    ;
    ...
    lattice->nbnds =
        end_band - start_band + 1;  // Number of bands to be computed
    ...
    // kpt_iredBZ stores cartesian coordinates of k-points in iBZ. (nibz, 3)
    ...
    if (Comm->commW_rank == 0)
    {
        ...
        // (9+3) 9 for R-matrix, 3 for tau (translational vector)
        ELPH_float* tau = sym_temp + 9 * lattice->nsym;

        ELPH_float* kiBZtmp = malloc(sizeof(ELPH_float) * 3 * nibz);
        CHECK_ALLOC(kiBZtmp);

        quick_read(dbid, "LATTICE_VECTORS", lattice->alat_vec);  // (3,3)
        quick_read(dbid, "SYMMETRY",
                   sym_temp);  // transpose is read (nsym, 3,3)
        quick_read(dbid, "K-POINTS",
                   kiBZtmp);  // (3,nibz) (in units of lat_param/2pi)
        ...
        */
        ...
        {
            tau[i] = 0;  // Setting fractional translation vector to zero
        }

        /* Get kpoints to cart coordinates */
        for (ND_int i = 0; i < nibz; ++i)
        {
            // Convert to units of 2*pi/L_i
        ...
        {
            // Set Rmat (3x3 rotational matrix)
            transpose3x3f(sym_temp + 9 * i, lattice->syms[i].Rmat);

            // Set tau (3-vector for fractional translation)
            memcpy(lattice->syms[i].tau, tau + 3 * i, sizeof(ELPH_float) * 3);

            /* fill the time-rev array*/
            if (i >= (lattice->nsym / (lattice->timerev + 1)))
            {
                // This logic identifies time-reversed symmetries based on the
                // total number of symmetries and timerev flag.
    ...
    // Broadcast iBZ k-points (nibz, 3)
    mpi_error = MPI_Bcast(lattice->kpt_iredBZ, 3 * nibz, ELPH_MPI_float, 0,
                          Comm->commW);
    MPI_error_msg(mpi_error);

    // Broadcast direct lattice vectors (3, 3)
    mpi_error = MPI_Bcast(lattice->alat_vec, 9, ELPH_MPI_float, 0, Comm->commW);
    MPI_error_msg(mpi_error);

    // compute reciprocal vectors and volume
    // blat_vec are reciprocal lattice vectors scaled by 2*pi. (3, 3)
    reciprocal_vecs(lattice->alat_vec, lattice->blat_vec);
    // b[:,i]  are blat. blat comes with 2*pi factor
    lattice->volume =
        fabs(det3x3(lattice->alat_vec));  // Volume of the unit cell

    // Allocate storage for k-points in the full BZ
    lattice->kpt_fullBZ_crys =
        calloc(3 * nkBZ, sizeof(ELPH_float));  // (nkBZ, 3) in crystal coords
    CHECK_ALLOC(lattice->kpt_fullBZ_crys);

    lattice->kpt_fullBZ =
        calloc(3 * nkBZ, sizeof(ELPH_float));  // (nkBZ, 3) in cartesian coords
    CHECK_ALLOC(lattice->kpt_fullBZ);

    // kmap stores index mapping: (nkBZ, 2) -> (iBZ index, symmetry index)
    lattice->kmap = malloc(sizeof(int) * nkBZ * 2);
    CHECK_ALLOC(lattice->kmap);

    // Expands the k-points from iBZ to the full BZ using symmetries
    ...
    // convert k-points from crystal to cartesian units (blat_vec contains
    // $2\pi$)
    // $\mathbf{k}_{cart} = \frac{1}{2\pi} \mathbf{b} \cdot \mathbf{k}_{crys}$
    ...
    pseudo->ntype =
        rint(ntype);  // Number of distinct atomic species (pseudo-potentials)
    ...
    if (Comm->commW_rank == 0)
    {
        ...
        quick_read(dbid, "N_ATOMS",
                   natom_per_type);  // Number of atoms for each species (ntype)
        quick_read(
            dbid, "atomic_numbers",
            atomic_numbers);  // Atomic number (Z) for each species (ntype)

        lattice->natom = 0;  // Bcast
        for (ND_int ia = 0; ia < pseudo->ntype; ++ia)
        {
            lattice->natom +=
                (int)rint(natom_per_type[ia]);  // Total number of atoms
        }
        ...
        quick_read(
            dbid, "ATOM_MAP",
            atomic_map);  // Mapping from species/index to global atom index
        quick_read(dbid, "ATOM_POS",
                   atom_pos_temp);  // Atomic positions (ntype, nspec_max, 3)

        lattice->atom_type = malloc(
            sizeof(int) *
            lattice->natom);  // Bcast: Type of pseudo-potential for each atom
        ...
        //
        lattice->atomic_pos =
            malloc(sizeof(ELPH_float) * 3 *
                   lattice->natom);  // Bcast: Atomic positions (natom, 3)
        ...
        for (ND_int it = 0; it < pseudo->ntype; ++it)
        {
            ...
            {
                // Atom index in global list (1-based from file, subtract 1)
                ...
                // Rearrange positions to (natom, 3) order
                memcpy(set_ptr, get_ptr, 3 * sizeof(ELPH_float));
                // Set the type (0 to ntype-1) for this atom
    ...
    *wfcs = malloc(sizeof(struct WFC) * nibz);  // wfcs in iBZ
    ...
    if (Comm->commW_rank == 0)
    {
        quick_read(dbid, "WFC_NG",
                   nGmax);  // Number of G-vectors for each k-point (nibz)
    }
    ...
    // dimensions[]
    ND_int ng_total = rint(dimensions[7]);  // Total number of unique G-vectors
    ND_int ng_shell =
        rint(dimensions[8]);  // Max number of G-vectors for any k-point
    ELPH_float* totalGvecs =
        malloc(sizeof(ELPH_float) * 3 *
               ng_total);  // (ng_total, 3) G-vectors in crystal coordinates
    CHECK_ALLOC(totalGvecs);

    ELPH_float* Gvecidxs =
        malloc(sizeof(ELPH_float) * nibz *
               ng_shell);  // (nibz, ng_shell) indices into totalGvecs (1-based)
    CHECK_ALLOC(Gvecidxs);

    if (Comm->commW_rank == 0)
    {
        quick_read(dbid, "G-VECTORS",
                   totalGvecs);  // (3, ng_total) - Transposed in file
        quick_read(dbid, "WFC_GRID",
                   Gvecidxs);  // (ng_shell, nibz) - Transposed in file
    ...
    if (Comm->commW_rank == 0)
    {
...

        {
            ...
            pseudo->nltimesj =
                rint(kb_pars[2]);  // Total number of KB projectors
            pseudo->lmax =
                rint(kb_pars[3]);  // Max angular momentum quantum number Lmax
                                   // (stored as Lmax+1 in Yambo)
        }
    }
    ...
    ND_int total_kpools =
        Comm->nkpools *
        Comm->nqpools;  // Total number of I/O pools (Comm->commR size)
    ...
    for (ND_int ik = 0; ik < nibz; ++ik)
    {
        ...
        // Calculate the slice of G-vectors for this process in commK
        ...
        (wfc_temp + ik)->gvec =
            malloc(sizeof(ELPH_float) * 3 * pw_this_cpu);  // (npw_loc, 3)
        CHECK_ALLOC((wfc_temp + ik)->gvec);

        // Populate the G-vectors for the local slice
        ...
        // (nspin, bands, nspinor, npw_loc) in memory
        ...
        {
            // The process in the I/O pool handles the parallel read over commK
        ...
        //(nltimesj, ntype, npw_loc) in memory
    ...
    for (ND_int ik = 0; ik < nibz; ++ik)
    {
        ...
        ND_int spin_stride_len =
            lattice->nbnds * lattice->nspinor * pw_this_cpu;
        // Bcast the wfc (local slice over commK) from the I/O pool to all
        // others in commR
        mpi_error =
        ...
        // broadcast kb projector (local slice over commK)
    ...
    if (Comm->commW_rank == 0)
    {
        for (ND_int ipot1 = 0; ipot1 < pseudo->ntype; ++ipot1)
        {
            ...
            {
                // Compare element symbol from UPF file to element symbol from
                // ns.db1
                if (strcmp(temp_ele, atom_symbols + 3 * ipot2) == 0 &&
                    !check_ele_in_array(pseudo_order, pseudo->ntype, ipot2))
                {
                    found = true;
                    // pseudo_order[ipot1] maps the UPF file index (ipot1) to
                    // the ns.db1 species index (ipot2)
    ...
    if (Comm->commW_rank == 0)
    {
        ...
        {
            // iorder is the species index for the current pseudopotential
            ND_int iorder = pseudo_order[ipot];

            cwk_path_join(ph_save_dir, pseudo_pots[ipot], temp_str,
                          temp_str_len);

            // Parse the UPF file and store data in the correct species index
            // (iorder)
    ...
    pseudo->ngrid_max = find_maxint(
        pseudo_order,
        pseudo->ntype);  // Max number of grid points for any local potential
    lattice->npw_max =
        find_maxfloat(nGmax, nibz);  // Max number of G-vectors for any k-point
    ...
    // qmax_abs is set to the maximum magnitude of any q-vector plus one.
    pseudo->vloc_table->qmax_abs = ceil(fabs(qmax_val)) + 1;
    // Note this must be called in the last else U.B
    create_vlocg_table(lattice, pseudo,
                       Comm);  // Compute the Vloc(G) interpolation table
...
/**
 * @brief Frees all dynamically allocated memory within the @ref Phonon
 * structure.
 *
 * @param phonon Pointer to the Phonon structure.
 */
...
/**
 * @brief Frees all dynamically allocated memory for wavefunctions, lattice,
 * pseudo, and phonon data.
 *
 * @param wfcs Pointer to the array of WFC structures.
 * @param lattice Pointer to the Lattice structure.
 * @param pseudo Pointer to the Pseudo structure.
 * @param phonon Pointer to the Phonon structure.
 */
...
// ============ static functions
/**
 * @brief Sets the G-vectors for a specific k-point's local slice.
 *
 * Retrieves the G-vector indices from `Gvecidxs` (for k-point `ik` starting at
 * `nG_shift`), uses these indices to fetch the corresponding global G-vectors
 * from `totalGvecs`, and converts them to cartesian coordinates using
 * `lat_param`.
 *
 * @param gvec Pointer to the output buffer for local G-vectors (nG, 3).
 * @param ik Index of the k-point in the irreducible BZ (iBZ).
    ...
    const ELPH_float* lat_param, const ND_int nG, const ND_int nG_shift)
{
    ...
    {
        // Gvecidxs stores 1-based indices, subtract 1 for 0-based
        ND_int gidx = rint(gidx_temp[ig + nG_shift] - 1);
        if (gidx < 0)
        {
            error_msg("Wrong g indices");
        }
        ELPH_float* gvec_temp =
            gvec + ig * 3;  // current local G-vector (3 components)

        for (int i = 0; i < 3; ++i)
        {
            // Convert from crystal coordinates (in $2\pi/a_i$) to cartesian (in
            // $2\pi/L_i$) totalGvecs is stored as (3, ng_total) in the file,
            // accessed as totalGvecs[component + gidx * 3] Correction: The file
            // format is (3, ng_total). In C, this is stored as
            // totalGvecs[component*ng_total + gidx]. But the code uses
            // totalGvecs[ng_total * i + gidx]. Assuming i is component index:
            // totalGvecs[component*ng_total + gidx] is component $i$ of
            // G-vector $gidx$.
 ...
 * k-point.
 *
 * The file is opened using parallel I/O (`nc_open_par`) across `comm`.
 * The data is read collectively. The NetCDF variable is typically 4D:
 * (bands, nspinor, npw, 2 [real/imag]).
 * The read slice dimensions correspond to:
 * (bands\_to\_read, nspinor, npw\_loc, 2 [real/imag])
 *
 * @param spin_stride_len The stride length in the output buffer for a single
 * spin component: $\text{nbnds} \times \text{nspinor} \times \text{nG}$.
 * @param ik Index of the k-point in the irreducible BZ (iBZ).
 ...
 * @param work_array Temporary buffer for path/variable names.
 * @param work_array_len Length of the `work_array` buffer.
 * @param out_wfc Output buffer for wavefunctions. The slice for spin `is`
 * starts at $\text{out\_wfc} + \text{is} \times \text{spin\_stride\_len}$.
 * @param comm MPI Communicator for G-vector parallelization (`Comm->commK`).
 */
static void get_wfc_from_save(ND_int spin_stride_len, ND_int ik, ND_int nkiBZ,
                              ND_int nspin, ND_int nspinor, ND_int start_band,
                              ...
                              MPI_Comm comm)
{
    ...
    {
        char tmp_buf[64];
        // Filename format: ns.wf_fragments_<spin_index*nkiBZ + k_index+1>_1
        ...
        //// (bands, nspinor, npw, 2 [real/imag]) - NetCDF dimensions
        size_t startp[4] = {start_band - 1, 0, G_shift,
                            0};  // band index is 1-based, converted to 0-based
        size_t countp[4] = {
            nbnds, nspinor, nG,
            2};  // Read `nbnds` bands, all `nspinor` components, `nG`
                 // G-vectors, and 2 components (real/imag)
 ...
 * in serial mode.
 *
 * This function is meant for serial I/O (collective is not used) and is
 * typically called by a single process (e.g., $W$-rank 0) before broadcasting
 * the result.
 *
 * @param ncid NetCDF file ID.
 * @param var_name Name of the variable to read.
...
/**
 * @brief Reads a sub-array (slice) of a specified variable from an open NetCDF
 * file using collective parallel I/O.
 *
 * The access mode is set to `NC_COLLECTIVE` for parallel I/O.
 *
 * @param ncid NetCDF file ID.
 * @param var_name Name of the variable to read from.
 * @param startp Array of starting indices for the slice (NetCDF dimension
...
/**
 * @brief Calculates which I/O pool should read the wavefunction for a specific
 * k-point.
 *
 * Divides the k-points among `total_kpools` processes based on the quotient $q$
 * and remainder $r$ of the division $N_{iBZ} / N_{pools}$. The first $r$ pools
 * read $q+1$ k-points, and the remaining $N_{pools} - r$ pools read $q$
 * k-points.
 *
 * @param ik K-point index in the irreducible BZ (0-based).
 * @param q Quotient $N_{iBZ} / N_{pools}$.
 * @param r Remainder $N_{iBZ} \pmod{N_{pools}}$.
 * @return The rank of the I/O pool.
 */
static inline ND_int get_wf_io_pool(ND_int ik, ND_int q, ND_int r)
{
    ...
    ND_int offset =
        r *
        (q +
         1);  // Index boundary between pools reading q+1 and pools reading q
    if (ik < offset)
    {
        // One of the first 'r' pools, each handling 'q+1' k-points
        return ik / (q + 1);
    }
    else
    {
        // One of the remaining 'N_pools - r' pools, each handling 'q' k-points
        // Subtract the k-points handled by the first 'r' pools, then divide by
        // 'q', and add 'r' for the rank offset.
        return (ik - r) / q;
    }
}
