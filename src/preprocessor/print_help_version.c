...
void ELPH_print_help(void)
{
    ...
    fprintf(stdout,
            "--code     : DFT code. currently only \"qe\" is supported."
            "Default is \"qe\"\n");
    fprintf(stdout, "\n");
    fprintf(stdout,
            "-pp        : activates preprocessor. used for ph_save creation.\n"
            "If -pp is absent, the code assumes that user requested elph "
            "calculation.\n");
    fprintf(stdout, "\n");
    ...
    fprintf(
        stdout,
        "To perform the elph calculation, an example command looks like:\n");
    ...
    fprintf(stdout, "\n");
    fprintf(stdout,
            "ELPH_COPY_CMD : Default (\"cp\") In case if you want to \n"
            "create a softlink to dfpt data, use can set this varibale\n");
    fprintf(stdout,
            "To use symlinks (instead of copy), use export ELPH_COPY_CMD=\"ln "
            "-sr\". Symlinks work only on linux \n");
    fprintf(stdout, "\n");
    fprintf(stdout,
            "ELPH_SAVE_DIR : Default (ph_save). In case if you want to"
