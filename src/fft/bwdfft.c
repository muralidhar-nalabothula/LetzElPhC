/**
 * @file
 * @brief Parallel inverse 3D FFT from G-space to real space
 *
 * This file implements parallel inverse FFT transforms converting wavefunctions
 * from reciprocal (G-space) to real space.
 */

#include <complex.h>
#include <fftw3.h>
#include <stdbool.h>
#include <string.h>

#include "common/error.h"
#include "elphC.h"
#include "fft.h"

/**
 * @brief Performs parallel inverse 3D FFT from G-space to real space
 *
 * Transforms wavefunctions from reciprocal space (G-vectors) to real space
 * grid. The algorithm proceeds as:
 *
 * a) Map G-vectors from sphere to FFT box (sphere to box)
 * b) Inverse FFT along Z direction
 * c) Transpose data: gather (Gx,Gy) pairs and scatter Z slices across MPI ranks
 * d) Inverse FFT along Y and X directions
 *
 * @param[in,out] plan FFT plan containing buffers, dimensions, and FFTW plans
 * @param[in] nsets Number of wavefunctions to transform
 * @param[in] wfcG Input wavefunctions in G-space, shape: (nsets, ngvecs_loc)
 * @param[out] wfcr Output wavefunctions in real space, shape: (nsets, Nx, Ny,
 * nzloc)
 * @param[in] conjugate If true, conjugate the input before FFT
 *
 * @note Input wfcG is never modified, even when conjugate is true
 * @note Output is NOT normalized (FFT convention dependent)
 */
void invfft3D(struct ELPH_fft_plan* plan, const ND_int nsets, ELPH_cmplx* wfcG,
              ELPH_cmplx* wfcr, const bool conjugate)
{
    // first some basic stuff
    ND_int Nx = plan->fft_dims[0];
    ND_int Ny = plan->fft_dims[1];
    ND_int Nz = plan->fft_dims[2];
    ND_int fft_buf_size = Nx * Ny * plan->nzloc;
    ND_int Ny_stride = plan->nzloc * Ny;
    for (ND_int iset = 0; iset < nsets; ++iset)
    {
        ELPH_cmplx* wfcG_tmp = wfcG + iset * plan->ngvecs_loc;
        ELPH_cmplx* wfcr_tmp = wfcr + iset * fft_buf_size;
        ND_int igvec = 0;
        for (ND_int ixy = 0; ixy < plan->nGxyloc; ++ixy)
        {
            ELPH_cmplx* zfft_ptr = plan->nz_buf + ixy * Nz;
            // a) sphere to box
            for (ND_int iz = 0; iz < Nz; ++iz)
            {
                zfft_ptr[iz] = 0;
            }
            for (ND_int ig = 0; ig < plan->ngxy_z[ixy]; ++ig)
            {
                ND_int Gz = plan->gvecs[3 * igvec + 2];
                if (Gz < 0)
                {
                    Gz += Nz;
                }
                zfft_ptr[Gz] = wfcG_tmp[igvec];
                if (conjugate)
                {
                    zfft_ptr[Gz] = conj(zfft_ptr[Gz]);
                }
                ++igvec;
            }
        }
        if (igvec != plan->ngvecs_loc)
        {
            error_msg("Gvec mismatch in bwd execute.");
        }
        // b) perform invfft along z
        fftw_fun(execute)(plan->bplan_z);
        // c) (i) transpose the data
        bwd_transpose(plan);
        // zero out the xy buffer and fill the fft_data buffer
        for (ND_int ixyz = 0; ixyz < fft_buf_size; ++ixyz)
        {
            wfcr_tmp[ixyz] = 0;
        }
        for (ND_int ixy = 0; ixy < plan->nGxy; ++ixy)
        {
            ND_int Gx = plan->Gxy_total[2 * ixy];
            ND_int Gy = plan->Gxy_total[2 * ixy + 1];
            // [Gx,Gy,0] =  Gx*Ny*Nzloc + Gy*Nzloc
            ELPH_cmplx* xy_buf = wfcr_tmp + plan->nzloc * (Gy + Gx * Ny);
            // (Nxy,Zloc)
            memcpy(xy_buf, plan->nz_buf + ixy * plan->nzloc,
                   sizeof(ELPH_cmplx) * plan->nzloc);
        }
        // c) (ii) FFT along Y
        for (ND_int ix = 0; ix < plan->fft_dims[0]; ++ix)
        {
            if (!(plan->gx_inGvecs[ix]))
            {
                continue;
            }
            ELPH_cmplx* wfcr_tmp_y = wfcr_tmp + ix * Ny_stride;
            ND_int iax = fftw_fun(alignment_of)((void*)wfcr_tmp_y);
            iax /= sizeof(ELPH_cmplx);
            fftw_fun(execute_dft)(plan->bplan_y[iax], wfcr_tmp_y, wfcr_tmp_y);
        }
        ND_int ia = fftw_fun(alignment_of)((void*)wfcr_tmp);
        ia /= sizeof(ELPH_cmplx);
        // d) FFT along X
        fftw_fun(execute_dft)(plan->bplan_x[ia], wfcr_tmp, wfcr_tmp);
    }
}
