                      ...
                      const struct ELPH_MPI_Comms* Comm)
{
    ...
    {
        print_info_msg(Comm->commW_rank, "DFT Code                : %s",
                       "Quantum ESPRESSO");
    }
    else
    {
    ...
    print_info_msg(Comm->commW_rank, "ph_save Dir             : %s",
                   ph_save_dir);
    print_info_msg(Comm->commW_rank, "Screening Kernel        : %s", kernel);
    print_info_msg(Comm->commW_rank, "ELPH Output convention  : %s",
                   convention);
    ...
    if (omp_n_threads)
    {
        print_info_msg(Comm->commW_rank, "Openmp Threads          : %s",
                       omp_n_threads);
    }
    else
    {
                        ...
                        const struct Lattice* lattice)
{
    ...
    print_info_msg(Comm->commW_rank,
                   "============== Lattice info ==============");
    print_info_msg(Comm->commW_rank, "Number of atoms in Unit Cell  : %d",
                   (int)lattice->natom);
    print_info_msg(Comm->commW_rank, "Number of spin components     : %d",
                   (int)lattice->nspin);
    print_info_msg(Comm->commW_rank, "Number of spinor components   : %d",
                   (int)lattice->nspinor);

    print_info_msg(Comm->commW_rank, "Dimension of system           : %cD",
                   lattice->dimension);
    //
    ...
    {
        print_info_msg(Comm->commW_rank,
                       "Magnetic calculation type     : Non-magnetic");
    }
    else if (lattice->nmag == 2)
    {
        print_info_msg(Comm->commW_rank,
                       "Magnetic calculation type     : LSDA (collinear spin)");
    }
    else
    {
        print_info_msg(
            Comm->commW_rank,
            "Spin orbit coupling           : magnetic non-collinear");
    }
    //
    print_info_msg(Comm->commW_rank, "Number of kpoints in iBZ      : %d",
                   (int)lattice->nkpts_iBZ);
    print_info_msg(Comm->commW_rank, "Number of kpoints in full BZ  : %d",
                   (int)lattice->nkpts_BZ);
    print_info_msg(Comm->commW_rank, "Number of symmetries          : %d",
                   (int)lattice->nsym);
    //
    if (lattice->timerev == 1)
    {
        print_info_msg(Comm->commW_rank,
                       "Time reversal for electrons   : True");
    }
    else
    {
        print_info_msg(Comm->commW_rank,
                       "Time reversal for electrons   : False");
    }
    //
    print_info_msg(Comm->commW_rank,
                   "FFT Grid                      : %d  %d  %d",
                   (int)lattice->fft_dims[0], (int)lattice->fft_dims[1],
                       ...
                       const struct Phonon* phonon)
{
    ...
    print_info_msg(Comm->commW_rank, "Number of qpoints in iBZ      : %d",
                   (int)phonon->nq_iBZ);
    print_info_msg(Comm->commW_rank, "Number of qpoints in full BZ  : %d",
                   (int)phonon->nq_BZ);
    print_info_msg(Comm->commW_rank, "Number of phonon symmetries   : %d",
                   (int)phonon->nph_sym);
    ...
    {
        print_info_msg(Comm->commW_rank,
                       "Time reversal for phonons     : True");
    }
    else
    {
        print_info_msg(Comm->commW_rank,
                       "Time reversal for phonons     : False");
    }
}
